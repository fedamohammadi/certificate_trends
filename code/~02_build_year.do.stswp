...
* Variable naming changed around 2011. Handle both.
*-------------------------------------------------------------------------------
import delimited "$raw\ipeds\c`yr'_a_data_stata.csv", clear varnames(1) stringcols(2)
rename *, lower

* Build ctotalt if it doesn't exist (older files use crace15 + crace16)
capture confirm variable ctotalt
if _rc != 0 {
    display "  Note: `yr' uses old CRACE format. Building ctotalt = crace15 + crace16."
...
*   - 2000-2010ish: "1.0101" (decimal, no leading zero)
*   - 2011ish-2024: "010101" (no decimal, leading zero)
* Normalize to 6-digit no-decimal string, THEN drop summary rows.
*-------------------------------------------------------------------------------
replace cipcode = strtrim(cipcode)

* Remove decimal if present ("1.0101" -> "10101")
replace cipcode = subinstr(cipcode, ".", "", .)

* Pad with leading zeros to 6 characters ("10101" -> "010101")
replace cipcode = string(real(cipcode), "%06.0f")

* Now drop summary rows in the normalized format
